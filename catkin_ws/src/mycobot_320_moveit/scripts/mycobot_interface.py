...
class MycobotInterface(object):
    ...
    def __init__(self):
        ...
        self.model = rospy.get_param("~model", "320")
        self.vel_rate = rospy.get_param("~vel_rate", 32.0)  # bit/rad
        self.min_vel = rospy.get_param(
            "~min_vel", 10
        )  # bit, for the bad velocity tracking of mycobot.
        ...
        self.joint_command_sub = rospy.Subscriber(
            "joint_command", JointState, self.joint_command_cb
        )
        ...
        self.open_gripper_srv = rospy.Service(
            "open_gripper", Empty, self.open_gripper_cb
        )
        self.close_gripper_srv = rospy.Service(
            "close_gripper", Empty, self.close_gripper_cb
        )
        ...
        while not rospy.is_shutdown():
            ...
            if self.real_angles:
                ...
                for i, ang in enumerate(self.real_angles):
                    msg.name.append("mycobot_arm_joint_" + str(i))
    ...
    def joint_as_cb(self, goal):
        ...
        for i in range(len(points)):
            ...
            if goal.trajectory.header.stamp == rospy.Time():
                ...
                seg["start_time"] = (
                    goal.trajectory.header.stamp + points[i].time_from_start
                ) - durations[i]

            seg["end_time"] = seg["start_time"] + durations[i]

            # Checks that the incoming segment has the right number of elements.
            if len(points[i].velocities) > 0 and len(points[i].velocities) != len(
        ...
        ## wait for start
        rospy.loginfo(
            "Trajectory start requested at %.3lf, waiting...",
            goal.trajectory.header.stamp.to_sec(),
        )
        r = rospy.Rate(100)
        ...
        for i, seg in enumerate(trajectory):
            ...
            if (
                len(trajectory) == 1
            ):  # only has the goal angle position, calculate the average velocity
                vel = (
                    (target_angles - np.array(self.real_angles))
                    / durations[i].to_sec()
                    * np.pi
                    / 180.0
                )
                ...
                if (
                    vel < self.min_vel
                ):  # workaround to solve the bad velocity tracking of mycobot
                    vel = self.min_vel
            ...
            while time.to_sec() < seg["end_time"].to_sec():
                ...
                rospy.logdebug(
                    ...
                    i,
                    (durations[i] - (time - seg["start_time"])).to_sec(),
                    time.to_sec(),
                )
                ...
                if self.joint_as.is_preempt_requested():
                    ...
                    if self.joint_as.is_new_goal_available():
                        rospy.logwarn(
                            "New trajectory received. Aborting old trajectory."
                        )
                    else:
                ...
                if (time - feedback.header.stamp).to_sec() > 0.1:  # 10 Hz
                    ...
                    feedback.error.positions = (
                        (target_angles - actual_angles) / 180 * np.pi
                    ).tolist()
            ...
            for tol in goal.path_tolerance:
                ...
                if tol.position > 0 and pos_err > tol.position:
                    msg = (
                        "Unsatisfied position tolerance for "
                        + tol.name
                        + ", trajectory point"
                        + str(i + 1)
                        + ", "
    ...
    def gripper_as_cb(self, goal):
        ...
        self.mc.set_gripper_state(
            goal_state, 100
        )  # first arg is the flag 0 - open, 1 - close; second arg is the speed
        self.lock.release()
        ...
        while not rospy.is_shutdown():
            ...
            rospy.logdebug(
                "Current gripper value is  %d state is %d",
                self.gripper_value,
                self.gripper_is_moving,
            )
