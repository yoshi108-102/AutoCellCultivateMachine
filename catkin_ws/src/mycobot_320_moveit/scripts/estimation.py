...
class CVEstimator:
    def __init__(self):
        ...
        self.pipettePosePublisher = rospy.Publisher("target_estimation",PoseStamped,queue_size=10)
        self.armPosePublisher = rospy.Publisher("arm_estimation",PoseArray,queue_size=10)
    def yolo_init(self):
    ...
    def displayPipettePose(self,color_image):
        txt = "PipetteHead"
        x,y,z = self.pipettePose.pose.position.x,self.pipettePose.pose.position.y,self.pipettePose.pose.position.z
    ...
    def displayArmPose(self,color_image):
        for pose in self.armPose.poses:
            #poseの座標を画像上の座標に変換
            x,y = rs.rs2_project_point_to_pixel(self.color_intr,[pose.position.x,pose.position.y,pose.position.z])
            x,y = int(x),int(y)
            cv2.circle(color_image,(x,y),5,(0,255,0),-1)
            txt = "Arm"
            cv2.putText(color_image, txt, (x,y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0), 2)
