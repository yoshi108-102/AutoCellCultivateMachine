...
class TfBroadCaster:
    def __init__(self):
        ...
        self.objectTfSetter = rospy.Subscriber(
            "target_estimation", PoseStamped, self.set_object_tf
        )
        self.camera_trans = None
        self.handTfSetter = rospy.Subscriber(
            "hand_estimation", PoseArray, self.set_hand_tf
        )
    ...
    def get_camera_trans(self):
        try:
            self.camera_trans = self.buffer.lookup_transform(
                target_frame="mycobot_base_link",
        ...
        for pose in msg.poses:
            if pose.orientation == Quaternion(0, 0, 0, 0):
                ...
                pose = tf2_geometry_msgs.do_transform_pose(
                    pose_stamped, self.camera_trans
                )
