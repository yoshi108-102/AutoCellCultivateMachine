    ...
    for cur,dirs,files in os.walk(dataset):
        ...
        for file in files:
            if file.endswith(".jpg"):
                ...
                paht_list,id = list(os.path.split(paht_list))
                x_sum[int(id)] += iou
                x_num[int(id)] += 1
                iou_average += iou
                print(f"{file}: {iou}")
    iou_average /= file_num
    y = [x_sum[i] / x_num[i] for i in range(5)]
    plt.rcParams["font.size"] = 18
    plt.plot(x,y)
    plt.scatter(x,y,color="white",s=100)
    plt.axhline(y=0,color="black",linewidth=0.5)
    plt.title("accurancy level of arm detection")
    plt.xlabel("distance between camera center and arm (pixel 640x480)",fontdict={"size":12})
    plt.ylabel("IoU between detected arm and real")
    plt.savefig(os.path.join(PKG_PATH,"analysis","experiment4","iou_and_distance.png")
                ,bbox_inches="tight")
