...
class PictureTaker:
    def __init__(self):
        ...
        self.take_cnt = 0
        self.yamlData = {
            round(j * 50, 3): {i * 15: {"points": []} for i in range(1, 11)}
            for j in range(1, 6)
        }
    ...
    def mouseEvent(self, event, x, y, flags, param):
        ...
        dir_path = os.path.join(
            ...
            "experiment2_2",
            "pipette",
            str(distances[id]) + "pixel",
            str(angles[angle_id]) + "degree",
        )
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            try:
                ...
                with open(YAML_PATH, "w") as f:
                    self.yamlData[distances[id]][angles[angle_id]]["points"].append(
                        points
                    )
                    yaml.dump(self.yamlData, f)
                rospy.loginfo(
                    "Saved image to " + os.path.join(dir_path, str(length) + ".png")
                )
            except Exception as e:
                rospy.logwarn(
                    "No such key in yamlData or finished taking pictures or something else"
                )
                exit()
        elif event == cv2.EVENT_RBUTTONDOWN:
            exit()
            rospy.loginfo(
                "next image will be saved to "
                + os.path.join(dir_path, str(angles[self.take_cnt % take_num]) + ".png")
            )
