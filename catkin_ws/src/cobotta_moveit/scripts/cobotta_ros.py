...
class CobottaArmMoveit:
    def __init__(self, group_name: str = "arm"):
        ...
        self.modeSub = rospy.Subscriber("/cobotta/CurMode", Int32, self.modeCallback)
    ...
    def pipettePointCallback(self, msg: PoseStamped):
        ...
        endEffectorPose = self.tfListener.lookupTransform(
            "base_link", "J6"
        ...
        dist = math.sqrt(
            (ex - pose.pose.position.x) ** 2
            + (ey - pose.pose.position.y) ** 2
            + (ez - pose.pose.position.z) ** 2
        )
        if dist < 0.1:
