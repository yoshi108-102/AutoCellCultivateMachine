...
# define parameter ---------------------------------------------
USE_COBOTTA = True
WITH_COP = False         # center of rotation adjustment
...
pick_position = [0.019115, 0.162730, 0.347901, -0.999982, -0.000438, 0.002414, 0.005420]
...
class GripperNTLab(object):
    ...
    def gripper_move_up_and_down(self, offset):
        gripper_position = to_list(self.current_gripper_pose)
        direction = False
        if(self.cop_left.y<self.cop_right.y):
            direction = True
        i = 0
        i_max = 6
        limit_min = gripper_position[0] - offset
        limit_max = gripper_position[0] + offset
        if(all_close(self.cop_left.y,self.cop_right.y,0.1)):
            print("goal")
            i = i_max
        while i < i_max:
            if direction:
                while gripper_position[0] > limit_min:
                    gripper_position[0] -= 0.00005
                    gripper_position[2] += 0.00005
                    self.gripper_set_pose(gripper_position, 0)
                    self.gripper_execute()
                    rospy.sleep(0.08)
...
def main():
    try:
        ...
        if WITH_COP:
            ############################################################################
            # Adjust center of pressure
            ntlab.gripper_move_up_and_down(0.005)
            end_func("adjust cop executed")
            ############################################################################
        ############################################################################
        # Rotate
            # Cobotta------------------------------------------------------
